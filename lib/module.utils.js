...
export async function getFutureStream(...args) {
  ...
  if (!options.apiToken || options.apiToken === '') {
    log('No Streamify Token present in settings, please consult documentation.');
    return '';
  }
  if (!options.apiBaseUrl || options.apiBaseUrl === '') {
    log('No Streamify API URL present in settings, please consult documentation.');
    return '';
  }
  ...
  try {
    ...
    if (!res.ok) {
      log(`Response from Streamify API was not ok, please consult documentation. Status code: ${res.status}`);
      return '';
    }
